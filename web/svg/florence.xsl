...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		<svg xmlns="http://www.w3.org/2000/svg">
			<xsl:attribute name="width">
				<xsl:value-of select="(/flo:layout/flo:keyboard/flo:width + sum(/flo:layout/flo:extension[flo:placement != 'top']/flo:keyboard/flo:width)) * 20"/>
			</xsl:attribute>
			<xsl:attribute name="height">
				<xsl:value-of select="(/flo:layout/flo:keyboard/flo:height + sum(/flo:layout/flo:extension[flo:placement != 'left' and flo:placement != 'right']/flo:keyboard/flo:height)) * 20"/>
			</xsl:attribute>
	...
	<xsl:template name="key">
		<g class="key" width="40" height="40">
			<xsl:call-template name="coords">
				<xsl:with-param name="size" select="20"/>
			</xsl:call-template>
			<use class="shape" width="40" height="40" xlink:href="style.svg#s_default">
				<xsl:call-template name="coords">
					<xsl:with-param name="size" select="20"/>
				</xsl:call-template>
			...
			<xsl:if test="document('keymap.xml')">
				...
				<xsl:if test="document('keymap.xml')/keymap/key[@code=$actcode and @activated='yes']">
					<use class="selected" width="40" height="40" xlink:href="#s_default">
						<xsl:call-template name="coords">
							<xsl:with-param name="size" select="20"/>
						</xsl:call-template>
						<xsl:if test="string(flo:shape)">
...
<!--			<xsl:attribute name="viewbox">0 0 <xsl:value-of select="flo:width"/>
				...
				<xsl:when test="../flo:placement">
					...
					<xsl:choose>
						<xsl:when test="../flo:placement='top'">
							<xsl:value-of select="sum(/flo:layout/flo:extension[flo:placement='left']/flo:keyboard/flo:width) * 20"/>
						</xsl:when>
						<xsl:when test="../flo:placement='left'">
							<xsl:value-of select="sum(/flo:layout/flo:extension[flo:placement='left' and position() &lt; $position]/flo:keyboard/flo:width) * 20"/>
						</xsl:when>
						<xsl:when test="../flo:placement='right'">
							<xsl:value-of select="( sum(/flo:layout/flo:extension[flo:placement='right' and position() &lt; $position]/flo:keyboard/flo:width) + sum(/flo:layout/flo:extension[flo:placement='left']/flo:keyboard/flo:width) + /flo:layout/flo:keyboard/flo:width ) * 20"/>
						</xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
			...
			<xsl:choose>
				<xsl:when test="../flo:placement">
					...
					<xsl:choose>
						<xsl:when test="../flo:placement='top'">
							<xsl:value-of select="sum(/flo:layout/flo:extension[flo:placement='top' and position() &lt; $position]/flo:keyboard/flo:width) * 20"/>
						</xsl:when>
						<xsl:when test="../flo:placement='left'">
							<xsl:value-of select="sum(/flo:layout/flo:extension[flo:placement='top']/flo:keyboard/flo:height) * 20"/>
						</xsl:when>
						<xsl:when test="../flo:placement='right'">
							<xsl:value-of select="sum(/flo:layout/flo:extension[flo:placement='top']/flo:keyboard/flo:height) * 20"/>
						</xsl:when>
			...
			<xsl:attribute name="transform"><xsl:value-of select="concat('translate(',$x,', ',$y,')')"/></xsl:attribute>
